...
def confidence_plot(prediction, images, labels, class_index, max_row=5):
    ...
    for i in range(10):
        prediction_bool = (0.9 - i*0.1 < inferred_probability) & (inferred_probability < 1.0 - i*0.1)
        ...
        for j, confidence_image in enumerate(confidence_images):
            ...
            if confidence_labels[j] != class_index:
                cv2.putText(display_one, str(confidence_labels[j]), (10, 20), font, 1, (255,255,255))
            display[j*(height+20):(j+1)*(height+20), i*width:(i+1)*width] = display_one
    plt.figure()
